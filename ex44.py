...
# ”继承 vs 合成”的问题说到底还是关于代码重用的问题。你不想到处都是重复的代码，这样既
# 难看又没有效率。继承可以让你在基类里隐含父类的功能。而合成则是利用模块和别的类中的
# 函数调用实现了相同的目的。

# 如果两种方案都能解决重用的问题，那什么时候该用哪个呢？这个问题的答案其实是非常主观
# 的，不过我可以给你三个大体的指引方案：
#    1.不惜一切代价地避免多重继承，它带来的麻烦比能解决的问题都多。如果你非要用，那你
#      得准备好专研类得层次结构，以及花时间去找各种东西得来龙去脉吧。
#    2.如果你有一些代码会在不同位置和场景应用到，那就用合成来把它们做成模块。
#    3.只有在代码之间有清楚得关联，可以通过一个单独得共性联系起来得时候使用继承，或者
#      你受现有代码或者别得不可抗拒所限制非用不可的话，那也用吧。

# 然而，不要成为规则的奴隶。面向对象编程中要记住的一点是，程序员创建软件包，共享代码，
# 这些都是一种社交习俗。由于这是一种社交习俗，有时可能因为你的工作同事的原因，你需要
# 打破这些规则。这时候，你就需要去观察别人的工作方式，然后去适应这种场合。
